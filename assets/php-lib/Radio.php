...
class Radio implements JsonSerializable
{
     ...
     */
    public static function loadRadio($id)
    {
        $database = new Database();

        $gne = $database->select('*', self::radio_table, "WHERE id = $id LIMIT 1");
    ...
    private static function makeRadioFromDatabaseArray($radio_database)
    {
        try {
            ...
            $radio = new self($radio_database->name, $radio_database->url);
    ...
    public static function getAllRadios()
    {
        ...
        $gne = $database->select('*', self::radio_table);
     ...
     */
    public static function deleteRadio($id)
    {
        $database = new Database();
    ...
    public function save()
    {
        ...
        if ($this->created) {
            return $database->update(self::radio_table, ['name' => $this->name, 'url' => $this->url, 'cover_cached_token' => $this->coverToken], "`id` = $this->id");
        } else {
            $status = $database->insert(self::radio_table, ['name' => $this->name, 'url' => $this->url]);
